...
/**
 * Manual trigger to publish scheduled posts (for testing)
 * No authentication required for testing
 */
export async function GET(req: NextRequest) {
  try {
    console.log("[TEST PUBLISH] Starting manual publish check...");
    ...
    for await (const entity of entities) {
      ...
      console.log(`[TEST PUBLISH] Post ${entity.rowKey}:`);
      console.log(`  - Scheduled for: ${scheduledFor.toISOString()}`);
      console.log(`  - Current time:  ${now.toISOString()}`);
      console.log(`  - Should publish: ${scheduledFor <= now}`);

      // Check if it's time to publish
      if (scheduledFor <= now) {
        console.log(
          `[TEST PUBLISH] Publishing post ${entity.rowKey} for user ${entity.partitionKey}`
        );
        ...
        for (const platform of platforms) {
          console.log(`[TEST PUBLISH] Posting to ${platform}...`);
          const tokenData = await getToken(userId, platform);

          if (!tokenData?.accessToken) {
            console.log(`[TEST PUBLISH] No token for ${platform}`);
            postResults.push({
              platform,
              success: false,
          ...
          try {
            ...
            console.log(`[TEST PUBLISH] ${platform} result:`, result);
          } catch (error: any) {
            console.error(`[TEST PUBLISH] Error posting to ${platform}:`, error);
            result = {
              success: false,
              error: error.message,
              platform,
    ...
    console.log(`[TEST PUBLISH] Processed ${results.length} scheduled posts`);
